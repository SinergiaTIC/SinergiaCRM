...
$GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') Starting script remove subpanel form module stic_Financial_Products and stic_Transactions');

// HIDE MODULES FROM SUBPANELS IF MODULE IS NOT ENABLED ON THE MENU

// Set modules to hide
$modulesToHide = array(
    0 => 'stic_Financial_Products',
    1 => 'stic_Transactions',
);
...
$GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide Financial Management subpanel', $currentTabs);

$modulesToHideNotEnabled = array_diff($modulesToHide, $currentTabs);
$GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide Financial Management subpanel', $modulesToHideNotEnabled);
if (count($modulesToHideNotEnabled)> 0){
    ...
    $GLOBALS['log']->debug(__METHOD__.'('.__LINE__.') hide Financial Management subpanel', $unserialized);
