...
- `verifactu_cancel_id_c`: ID de la factura que se está rectificando
- `verifactu_rectified_date_c`: Fecha de expedición de la factura que se rectifica

**Validaciones implementadas:**
- En el logic hook `before_save` se valida que si una factura está marcada como rectificativa, todos los campos obligatorios estén cumplimentados (tipo, base, ID y fecha de la factura rectificada).
- Si faltan datos, se impide el guardado y se muestra un mensaje de error descriptivo.

**Lógica de envío a AEAT:**

**Ubicación**: `custom/modules/AOS_Invoices/SticUtils.php` - Método `sendToAeat()`

Cuando se envía una factura rectificativa a AEAT, el sistema:

1. **Determina el tipo de factura**: 
   - Si `verifactu_is_rectified_c` es true y hay `verifactu_rectified_base_c`, establece el `invoiceType` según el valor:
     - R1: Art 80.1 y 80.2
     - R2: Art. 80.3
     - R3: Art. 80.4
     - R4: Resto
...
2. **Incluye datos de rectificación**:
...
4. **Registra en la cadena**: La factura rectificativa se registra como un nuevo eslabón en la cadena de hash

**Errores AEAT comunes:**
- **Error 1117**: "TipoFactura" no es rectificativo pero se envía bloque FacturasRectificadas → Se resuelve estableciendo correctamente el `invoiceType` según `rectified_base`
- **Error 1118**: Falta bloque ImporteRectificacion para rectificativas por sustitución → Se resuelve enviando `correctedBaseAmount` y `correctedTaxAmount`
- **Error 1189**: Falta bloque Destinatarios para facturas R1-R4 → Verificar que el cliente tiene NIF informado
