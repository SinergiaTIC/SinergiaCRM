...
test('Accounts - minimal fields', async ({ page }) => {
  test.setTimeout(120000);
  await login(page);
  await page.goto(new URL('index.php?module=Accounts&action=EditView', BASE_URL).toString());
  await page.waitForLoadState('networkidle');
  await expandAll(page);
  const name = 'PWAccount_' + Math.random().toString(36).slice(2,8);
  await setFieldGeneric(page, ['input[name="name"]','input#name','input[name="account_name"]'], name);
  ...
  await page.goto(new URL(`index.php?module=Accounts&action=DetailView&record=${record}`, BASE_URL).toString());
