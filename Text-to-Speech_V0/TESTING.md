...
- [ ] **A6** — Texto muy largo (>2000 chars) → se reproduce completo (fragmentación automática)
...
- [ ] **B1** — Ir a una cuenta en vista detalle → ver "▶ Escuchar info destacada" en menú de acciones
- [ ] **B2** — Hacer clic → reproduce los campos configurados concatenados
- [ ] **B3** — Navegar a otra cuenta (PLAY activo) → sigue sonando sin interrupción
...
- [ ] **C1** — Ir a listado de cuentas → seleccionar 2-3 registros → Bulk Action → "Escuchar seleccionados"
...
- [ ] **N2** — PLAY activo → navegar al mismo módulo → sigue sonando sin corte
...
- [ ] **Ctl1** — Play/Pause toggle: icono cambia ▶ ↔ ⏸
- [ ] **Ctl2** — Stop: barra desaparece, audio se detiene, cache se limpia
- [ ] **Ctl3** — Velocidad 1.5x → el audio se acelera
- [ ] **Ctl4** — Velocidad 0.5x → el audio se ralentiza
- [ ] **Ctl5** — Velocidad 1x → normal
- [ ] **Ctl6** — Clic en barra de progreso → seek a esa posición
- [ ] **Ctl7** — Tiempo (mm:ss) se actualiza correctamente
- [ ] **Ctl8** — Playlist: desplegar menú → ver nombres reales de registros
...
- [ ] **Err1** — Superar límite diario → HTTP 429 + mensaje de error
- [ ] **Err2** — API key inválida → mensaje de error
- [ ] **Err3** — Sin conexión → mensaje de error genérico
- [ ] **Err4** — `TTS_ENABLED = 0` → botones TTS no aparecen
- [ ] **Err5** — `TTS_DEEPGRAM_API_KEY` vacío → botones TTS no aparecen
...
- [ ] **L10n3** — Interfaz en inglés → textos en inglés
- [ ] **L10n4** — Síntesis respeta `TTS_DEFAULT_LANGUAGE`
...
- [ ] **UX2** — Barra no se superpone con contenido
- [ ] **UX3** — Barra se oculta tras 5s de inactividad (CSS idle timeout)
- [ ] **UX4** — Botón de textarea aparece correctamente al lado del campo
...
|---------|-------|---------|--------|
| Configuración | 7 | | |
| Escenario A: Textarea | 7 | | |
| Escenario B: Detalle | 6 | | |
| Escenario C: Lista | 8 | | |
| Navegación | 6 | | |
| Controles | 8 | | |
| Barra de color | 4 | | |
