...
* **Orden de ejecución de las acciones**: El sistema ejecuta las acciones en el orden exacto en que aparecen en el flujo. Por defecto, las acciones automáticas se ordenan de la siguiente manera: primero se guardan **todos** los registros (acciones "Guardar registro") y, una vez todos tienen su identificador asignado, se crean las relaciones entre ellos (acciones "Enlazar registros"). **Nota importante para administradores avanzados:** Debido a este orden secuencial, los Flujos de Trabajo y LogicHooks que se configuren en los módulos implicados se ejecutarán durante el guardado inicial del registro, momento en el cual las relaciones con otros registros del formulario aún no se habrán creado. Si un Workflow necesita consultar la relación con otro registro creado en el mismo formulario, deberá configurarse para ejecutarse en un paso posterior al guardado inicial.
