...
* **Campos ocultos (vs Valores fijos)**: Se incorpora un nuevo tipo de campo visual: el campo oculto (`<input type="hidden">`). A diferencia de los "Valores fijos" (que se configuran y viven de forma segura exclusivamente en el servidor del CRM), los campos ocultos sí se inyectan en el código HTML del formulario.
