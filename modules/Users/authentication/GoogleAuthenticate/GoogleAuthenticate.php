...
class GoogleAuthenticate extends SugarAuthenticate
{
    ...
    public function pre_login()
    {
        ...
        $loginToken = [
            ...
            'google_auth_client_id' => $sugar_config['google_auth_client_id'] ?? '',
            'google_auth_only_form' => $sugar_config['google_auth_only_form'] ?? false,
        ];
        $loginToken = json_encode($loginToken);
        $_REQUEST['login_token'] = $loginToken;
    }
    ...
    public function loginAuthenticate($username, $password, $fallback=false, $PARAMS = array())
    {
        ...
        if (isset($_REQUEST['google_credentials']) && $_REQUEST['google_credentials'] != '') {
            ...
            $client->setClientSecret($sugar_config['google_signin_clientsecret']); // You can add this
            $payload = $client->verifyIdToken($_REQUEST['google_credentials']);

            if ($payload) {
                $email = $payload['email'] ?? '';

                $userBean = BeanFactory::newBean('Users');
	            $userBean->retrieve_by_email_address($email);
