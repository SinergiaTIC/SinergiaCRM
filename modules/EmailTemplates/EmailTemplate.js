...
setTrackerUrlSelectVisibility();});}
switch(action){case"convertLinksInTrackingUrls":const url='index.php?module=Campaigns&action=getCampaignTrackerURLs';fetch(url,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},body:new URLSearchParams({campaignID:campaignId})}).then(response=>response.json()).then(data=>{seenUrls=new Set();if(data){data.map(item=>seenUrls.add(item.url));}
fullHtml=tinymce.activeEditor.getContent();const doc=document.implementation.createHTMLDocument('temp');doc.documentElement.innerHTML=fullHtml;const links=doc.querySelectorAll('a');if(links.length>0){$('body').append(`
      <input type="text" id="url_text">
      <input type="text" id="tracker_url_add">
      <input type="checkbox" id="is_optout">
     `);let trackerURLsCreated=0;links.forEach((link)=>{const url=link.getAttribute('href');if(url&&!url.startsWith('{')&&!seenUrls.has(url)&&!url.includes('mailto')){seenUrls.add(url);linkName='Link_'+seenUrls.size;$('#url_text').val(linkName);$('#tracker_url_add').val(url);$('#is_optout').prop('checked',false);create();trackerURLsCreated++;fullHtml=fullHtml.replaceAll(url,"{"+linkName+"}");}});tinymce.activeEditor.setContent(fullHtml);const btn=document.getElementById('convertLinksInTrackingUrlsButton');if(btn){const msgEl=document.createElement('div');msgEl.id='convertLinksMsg';if(trackerURLsCreated!=0){msgEl.textContent=trackerURLsCreated+SUGAR.language.translate('Campaigns','LBL_CONVERT_LINKS_TO_TRACKING_URLS_MESSAGE_RESULTS');}else{msgEl.textContent=SUGAR.language.translate('Campaigns','LBL_CONVERT_LINKS_TO_TRACKING_URLS_MESSAGE_NO_RESULTS');}
msgEl.style.textAlign='center';msgEl.style.margin='1rem';msgEl.style.fontSize='1.2rem';msgEl.style.border='2px solid #b5bc31';btn.insertAdjacentElement('afterend',msgEl);btn.disabled=true;setTimeout(()=>{btn.disabled=false;const oldMsg=document.getElementById('convertLinksMsg');if(oldMsg){oldMsg.remove();}},5000);}}}).catch(error=>{console.error('Failed to get URLs tracker for the campaign with id = '+campaignId,error);});break;case"create":$('#url_text').val('');$('#tracker_name').val('');$('#tracker_url_add').val('');$('#is_optout').attr('checked',false);$('#tracker_url_add').removeAttr('disabled')
