...
if(!document.forms.EditView||document.forms.EditView.module.value!='EmailTemplates'){hideFieldErrorMessages();}
window.parent.$('.ui-dialog-content:visible').dialog('close');$.post('index.php?entryPoint=campaignTrackerSave',{module:'CampaignTrackers',record:'',action:'Save',campaign_id:_campaignId,tracker_name:trackerName,tracker_url:trackerURL,is_optout:$('#is_optout').prop('checked')?'on':'',response_json:true},function(resp){resp=JSON.parse(resp);if(resp.data.id){$('select[name="tracker_url"]').append('<option value="{'+trackerName+'}" data-id="'+resp.data.id+'" data-url="'+trackerURL+'">'+trackerName+' : '+trackerURL+'</option>');$('select[name="tracker_url"]').val('{'+trackerName+'}');$('#url_text').val('{'+trackerName+'}');}
if(!document.forms.EditView||document.forms.EditView.module.value!='EmailTemplates'){setTrackerUrlSelectVisibility();}});}
...
switch(action){case"convertLinksInTrackingUrls":fullHtml=tinymce.activeEditor.getContent();const doc=document.implementation.createHTMLDocument('temp');doc.documentElement.innerHTML=fullHtml;const links=doc.querySelectorAll('a');if(links.length>0){$('body').append(`
     <input type="text" id="url_text">
     <input type="text" id="tracker_url_add">
     <input type="checkbox" id="is_optout">
    `);const seenUrls=new Set();links.forEach((link)=>{const url=link.getAttribute('href');if(url&&!seenUrls.has(url)&&!url.includes('mailto')){seenUrls.add(url);linkName='Link_'+seenUrls.size;$('#url_text').val(linkName);$('#tracker_url_add').val(url);$('#is_optout').prop('checked',false);create();fullHtml=fullHtml.replaceAll(url,"{"+linkName+"}");}});tinymce.activeEditor.setContent(fullHtml);}
break;case"create":$('#url_text').val('');$('#tracker_name').val('');$('#tracker_url_add').val('');$('#is_optout').attr('checked',false);$('#tracker_url_add').removeAttr('disabled')
