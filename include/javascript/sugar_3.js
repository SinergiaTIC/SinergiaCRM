...
counter=0;$(".validation-message").each(function(){var $vtp=$(this).closest(".tab-pane-NOBOOTSTRAPTOGGLER");if($vtp.length){CurrentValidationTabId=$vtp.attr("id").split('-').slice(-1);}else{var $vtpl=$(this).closest("[class*='tab-panel-']");if($vtpl.length){var vtm=$vtpl.attr("class").match(/tab-panel-(\d+)/);if(vtm){CurrentValidationTabId=vtm[1];}}}$CurrentValidationTabA=$("#EditView_tabs a#tab"+CurrentValidationTabId);$CurrentValidationTabA.attr("style","background-color:red;").closest("li");if(counter==0){$CurrentValidationTabA.click();$CurrentValidationTabA.addClass("active");}
counter++;});}}
