...
SUGAR.isSupportedBrowser = function () {
  ...
  if (current && supported)
    return (
      checkMinSupported(current, String(supported.min)) &&
...
SUGAR.isIECompatibilityMode = function () {
  ...
  if (
    ...
    (agentStr.indexOf("Trident/5.0") > -1 || agentStr.indexOf("Trident/4.0") > -1)
  ) {
    mode = true;
  }
  return mode;
...
function isValidEmail(emailStr) {
  ...
  for (var i = 0; i < emailArr.length; i++) {
    ...
    if (trim(emailAddress) != "") {
      if (
        !/^\s*[\w.%+\-&'#!\$\*=\?\^_`\{\}~\/]+@([A-Z0-9-]+\.)*[A-Z0-9-]+\.[\w-]{2,}\s*$/i.test(emailAddress) &&
...
function isValidPhone(phoneStr) {
  ...
  if (!/^[0-9\-\(\)\s]+$/.test(phoneStr)) return false;
  return true;
}
...
function isDBName(str) {
  ...
  if (!/^[a-zA-Z][a-zA-Z\_0-9]*$/.test(str)) return false;
  return true;
}
...
function add_error_style(formname, input, txt, flash) {
  ...
  try {
    ...
    if (txt.substring(txt.length - 1) == ":") txt = txt.substring(0, txt.length - 1);
...
function clear_all_errors() {
  for (var wp = 0; wp < inputsWithErrors.length; wp++) {
    if (
      typeof inputsWithErrors[wp] != "undefined" &&
      typeof inputsWithErrors[wp].parentNode != "undefined" &&
  ...
  if (YAHOO.util.Dom.getAncestorByTagName(inputsWithErrors[0], "form")) {
    ...
    if (typeof window[formname + "_tabs"] != "undefined") {
      ...
      if (tabView.get) {
        ...
        for (var i in tabs) {
          if (typeof tabs[i] == "object") tabs[i].get("labelEl").style.color = "";
        }
      }
    }
...
function isFieldTypeExceptFromEmptyCheck(fieldType) {
  ...
  for (var i = 0; i < exemptList.length; i++) {
    if (fieldType == exemptList[i]) return true;
  }
  return results;
}
function validate_form(formname, startsWith) {
  ...
  for (var i = 0; i < validate[formname].length; i++) {
    if (validate[formname][i][nameIndex].indexOf(startsWith) == 0) {
      if (
        typeof form[validate[formname][i][nameIndex]] != "undefined" &&
        form[validate[formname][i][nameIndex]] instanceof HTMLElement &&
        typeof form[validate[formname][i][nameIndex]].value != "undefined"
      ) {
        var bail = false;
        if (
          !validate[formname][i][requiredIndex] &&
          trim(form[validate[formname][i][nameIndex]].value) == "" &&
          typeof validate[formname][i][jstypeIndex] != "undefined" &&
          validate[formname][i][jstypeIndex] != "binarydep" &&
          validate[formname][i][jstypeIndex] != "callback"
        ) {
          continue;
        }
        if (
          ...
          switch (validate[formname][i][typeIndex]) {
            ...
            case "teamset_mass":
              ...
              for (t in input_elements) {
                if (
                  input_elements[t].type &&
                  input_elements[t].type == "radio" &&
                  input_elements[t].checked == true &&
                  input_elements[t].value == "replace"
                ) {
                  var radio_elements = YAHOO.util.Selector.query(
                    "input[type=radio]",
                    document.getElementById(formname + "_team_name_table")
                  );
              ...
              if (replace_selected && !validation_passed) {
                add_error_style(
                  formname,
                  primary_field_id,
                  SUGAR.language.get("app_strings", "ERR_NO_PRIMARY_TEAM_SPECIFIED")
                );
                isError = true;
              }
              break;
            case "teamset":
              ...
              if (document.getElementById(table_element_id)) {
                ...
                for (t in input_elements) {
                  primary_field_id =
                    form[validate[formname][i][nameIndex]].name + "_collection_" + input_elements[t].value;
                  if (
                    input_elements[t].type &&
                    input_elements[t].type == "radio" &&
                    input_elements[t].checked == true
              ...
              case "range":
                if (
                  !inRange(
                    ...
                    validate[formname][i][maxIndex]
                  )
                ) {
                  isError = true;
                  var lbl_validate_range = SUGAR.language.get("app_strings", "LBL_VALIDATE_RANGE");
                  if (
                    typeof validate[formname][i][minIndex] == "number" &&
                    typeof validate[formname][i][maxIndex] == "number"
                  ) {
                    add_error_style(
                      formname,
                      validate[formname][i][nameIndex],
                      validate[formname][i][msgIndex] +
                        " " +
                        SUGAR.language.get("app_strings", "LBL_VALUE") +
                        " " +
                        form[validate[formname][i][nameIndex]].value +
                        " " +
                        lbl_validate_range +
                        " (" +
                        validate[formname][i][minIndex] +
                    ...
                    add_error_style(
                      ...
                      validate[formname][i][msgIndex] +
                        " " +
                        SUGAR.language.get("app_strings", "MSG_SHOULD_BE") +
                        " " +
                        validate[formname][i][minIndex] +
                        " " +
                        SUGAR.language.get("app_strings", "MSG_OR_GREATER")
                    );
                    ...
                    add_error_style(
                      ...
                      validate[formname][i][msgIndex] +
                        " " +
                        SUGAR.language.get("app_strings", "MSG_IS_MORE_THAN") +
                        " " +
                        validate[formname][i][maxIndex]
                    );
                  }
                    ...
                    if (trim(date1).length != 0 && !isBefore(date1, date2)) {
                      ...
                      add_error_style(
                        ...
                        validate[formname][i][msgIndex] +
                          ...
                          date1 +
                          ") " +
                          SUGAR.language.get("app_strings", "MSG_IS_NOT_BEFORE") +
                          " " +
                          date2
                      );
                    }
                  ...
                  if (value > maximum) {
                    ...
                    add_error_style(
                      ...
                      validate[formname][i][msgIndex] +
                        ...
                        SUGAR.language.get("app_strings", "MSG_IS_MORE_THAN") +
                        " " +
                        validate[formname][i][altMsgIndex]
                    );
                  }
                }
                break;
  ...
  if (formsWithFieldLogic) {
    ...
    if (formsWithFieldLogic.min && formsWithFieldLogic.max && formsWithFieldLogic._default) {
      ...
      var _default =
        formsWithFieldLogic._default.value != "" ? parseFloat(formsWithFieldLogic._default.value) : "undef";
...
function setPointer(theRow, theRowNum, theAction, theDefaultColor, thePointerColor, theMarkColor) {
  ...
  if (currentColor == "" || (currentColor != null && currentColor.toLowerCase() == theDefaultColor.toLowerCase())) {
    ...
    currentColor != null &&
    currentColor.toLowerCase() == thePointerColor.toLowerCase() &&
...
function goToUrl(selObj, goToLocation) {
  SUGAR.util.globalEval(
    "document.location.href = '" + goToLocation + "pos=" + selObj.options[selObj.selectedIndex].value + "'"
  );
}
...
function http_fetch_sync(url, post_data) {
  ...
  if (SUGAR.util.isLoginPage(global_xmlhttp.responseText)) return false;
  var args = {
...
function saveForms(savingStr, completeStr) {
  ...
  function success(data) {
    ...
    if (index == theForms.length) {
      ...
      if (saveAllButton) saveAllButton.style.visibility = "hidden";
    }
  }
...
sugarListView.prototype.confirm_action = function (del) {
  if (del == 1) {
    return confirm(
      SUGAR.language.get("app_strings", "NTC_DELETE_CONFIRMATION_NUM") +
        sugarListView.get_num_selected_string() +
        SUGAR.language.get("app_strings", "NTC_DELETE_SELECTED_RECORDS")
    );
  } else {
    if ($("#massupdate_form input#mass_duplicate").length == 1) {
      return confirm(
        SUGAR.language.get("app_strings", "LBL_MASS_DUPLICATE_UPDATE_CONFIRMATION_NUM") +
          sugarListView.get_num_selected_string() +
          SUGAR.language.get("app_strings", "NTC_DELETE_SELECTED_RECORDS")
      );
    } else {
      return confirm(
        SUGAR.language.get("app_strings", "NTC_UPDATE_CONFIRMATION_NUM") +
          sugarListView.get_num_selected_string() +
          SUGAR.language.get("app_strings", "NTC_DELETE_SELECTED_RECORDS")
      );
    }
...
sugarListView.prototype.use_external_mail_client = function (no_record_txt, module) {
  ...
  if (document.MassUpdate.select_entire_list.value == 1) {
    ...
    select = false;
  } else if (document.MassUpdate.massall.checked == true) select = false;
  else select = true;
...
sugarListView.prototype.use_external_mail_client_callback = function (o) {
  if (o.responseText) location.href = "mailto:" + o.responseText;
};
sugarListView.prototype.send_form = function (
  select,
  currentModule,
  action,
  no_record_txt,
  ...
  if (document.MassUpdate.select_entire_list.value == 1) {
    ...
    var href = action;
    if (action.indexOf("?") != -1) href += "&module=" + currentModule;
    else href += "?module=" + currentModule;
    if (return_info) href += return_info;
...
sugarListView.prototype.check_all = function (form, field, value, pageTotal) {
  ...
  if (pageTotal >= 0) sugarListView.update_count(pageTotal);
  else if (value) sugarListView.update_count(count, true);
...
sugarListView.prototype.check_boxes = function () {
  ...
  if (checkedCount > 0 && checkedCount == inputsCount) document.MassUpdate.massall.checked = true;
};
...
sugarListView.prototype.send_mass_update = function (mode, no_record_txt, del) {
  ...
  if (document.MassUpdate.select_entire_list && document.MassUpdate.select_entire_list.value == 1) mode = "entire";
  else mode = "selected";
  var ar = new Array();
  switch (mode) {
    case "selected":
      for (var wp = 0; wp < document.MassUpdate.elements.length; wp++) {
        var reg_for_existing_uid = new RegExp(
          "^" +
            RegExp.escape(document.MassUpdate.elements[wp].value) +
            "[s]*,|,[s]*" +
            RegExp.escape(document.MassUpdate.elements[wp].value) +
            "[s]*,|,[s]*" +
            RegExp.escape(document.MassUpdate.elements[wp].value) +
        ...
        if (
          typeof document.MassUpdate.elements[wp].name != "undefined" &&
      ...
      if (document.MassUpdate.uid.value != "" && ar.length) document.MassUpdate.uid.value += ",";
      ...
      if (
        typeof current_admin_id != "undefined" &&
        document.MassUpdate.module != "undefined" &&
        document.MassUpdate.module.value == "Users" &&
        (document.MassUpdate.is_admin.value != "" || document.MassUpdate.status.value != "")
      ) {
        var reg_for_current_admin_id = new RegExp(
          "^" +
            current_admin_id +
            "[s]*,|,[s]*" +
            current_admin_id +
  ...
  if (!sugarListView.confirm_action(del)) return false;
  if (del == 1) {
...
SUGAR.util = (function () {
  ...
  return {
    getAndRemove: function (el) {
      if (YAHOO && YAHOO.util && YAHOO.util.Dom) el = YAHOO.util.Dom.get(el);
    ...
    evalScript: function (text) {
      ...
      var objRegex = /<\s*script([^>]*)>((.|\s|\v|\0)*?)<\s*\/script\s*>/gim;
      var lastIndex = -1;
      ...
      while (result && result.index > lastIndex) {
        ...
        try {
          if (result[1].indexOf("src=") > -1) {
            var srcRegex = /.*src=['"]([a-zA-Z0-9_\-\&\/\.\?=:-]*)['"].*/gim;
            ...
            if (r1.test(srcResult) && srcResult.indexOf(window.location.hostname) == -1) {
              ...
              (function (srcResult) {
                $.ajax({ url: srcResult, async: false, method: "GET" }).done(function (responseText) {});
              })(srcResult);
            }
          } else {
            var srcRegex = /<!--([\s\S]*?)-->/;
    ...
    getAdditionalDetails: function (bean, id, spanId, show_buttons) {
      ...
      go = function () {
        ...
        success = function (data) {
          ...
          ajaxStatus.hideStatus();
          return oReturn(
            SUGAR.util.additionalDetailsCache[id]["body"],
            SUGAR.util.additionalDetailsCache[id]["caption"],
            SUGAR.util.additionalDetailsCache[id]["width"],
            SUGAR.util.additionalDetailsCache[id]["theme"]
          );
        };
        ...
        if (typeof SUGAR.util.additionalDetailsCalls[id] != "undefined") return;
        ajaxStatus.showStatus(SUGAR.language.get("app_strings", "LBL_LOADING"));
        url = "index.php?to_pdf=1&module=Home&action=AdditionalDetailsRetrieve&bean=" + bean + "&id=" + id;
        if (show_buttons) url += "&show_buttons=true";
    ...
    buildAccessKeyLabels: function () {
      if (typeof Y.env.ua !== "undefined") {
        ...
        for (i = 0; i < allButtons.length; i++) {
          if (
            allButtons[i].getAttribute("accesskey") &&
            allButtons[i].getAttribute("type") &&
            allButtons[i].getAttribute("type") == "button"
          ) {
            allButtons[i].setAttribute(
              "title",
              allButtons[i].getAttribute("title") + " [" + controlKey + allButtons[i].getAttribute("accesskey") + "]"
            );
          }
        }
  ...
  return {
    ...
    select: function (saved_search_select) {
      for (var wp = 0; wp < document.search_form.saved_search_select.options.length; wp++) {
        if (
          typeof document.search_form.saved_search_select.options[wp].value != "undefined" &&
    ...
    handleForm: function () {
      SUGAR.tabChooser.movementCallback = function (left_side, right_side) {
        ...
        for (i in left_side.childNodes) {
          if (
            typeof left_side.childNodes[i].nodeName != "undefined" &&
            left_side.childNodes[i].nodeName.toLowerCase() == "option" &&
            typeof SUGAR.savedViews.columnsMeta[left_side.childNodes[i].value] != "undefined" &&
            ...
            if (optionNode.value == SUGAR.savedViews.selectedOrderBy) selectedIndex = nodeCount;
          }
        }
  ...
  return {
    searchFormSelect: function (view, previousView) {
      ...
      if (document.getElementById(module + theView + "SearchForm").innerHTML == "") {
        ...
        url =
          "index.php?module=" + module + "&action=index&search_form_only=true&to_pdf=true&search_form_view=" + theView;
        var tpl = "";
        if (
          document.getElementById("search_tpl") != null &&
          typeof document.getElementById("search_tpl") != "undefined"
        ) {
        ...
        if (theView == "saved_views")
          url +=
            "&displayColumns=" +
            SUGAR.savedViews.displayColumns +
            "&hideTabs=" +
            SUGAR.savedViews.hideTabs +
            "&orderBy=" +
            SUGAR.savedViews.selectedOrderBy +
            "&sortOrder=" +
            SUGAR.savedViews.selectedSortOrder;
        var cObj = YAHOO.util.Connect.asyncRequest("GET", url, { success: success, failure: success });
      } else {
        handleDisplay();
      }
    ...
    clear_form: function (form, skipElementNames) {
      ...
      for (var i = 0; i < elemList.length; i++) {
        ...
        if (
          ...
          typeof skipElementNames != "undefined" &&
...
SUGAR.tabChooser = (function () {
  ...
  return {
    ...
    left_to_right: function (left_name, right_name, left_size, right_size) {
      ...
      frozen_options =
        frozen_options && frozen_options[left_name] && frozen_options[left_name][right_name]
          ? frozen_options[left_name][right_name]
          : [];
    ...
    right_to_left: function (left_name, right_name, left_size, right_size, max_left) {
      ...
      frozen_options =
        SUGAR.tabChooser.frozenOptions &&
        SUGAR.tabChooser.frozenOptions[right_name] &&
        SUGAR.tabChooser.frozenOptions[right_name][left_name]
          ? SUGAR.tabChooser.frozenOptions[right_name][left_name]
          : [];
      for (i = 0; i < hidden_columns_ref.options.length; i++) {
...
SUGAR.language = (function () {
  return {
    ...
    get: function (module, str) {
      if (
        typeof SUGAR.language.languages[module] == "undefined" ||
...
SUGAR.contextMenu = (function () {
  return {
    ...
    onContextMenuItemClick: function (p_sType, p_aArguments, p_oItem) {
      ...
      funct = SUGAR.util.globalEval(
        SUGAR.contextMenu.objectTypes[SUGAR.contextMenu.objects[id]["objectType"]]["menuItems"][this.index]["action"]
      );
    ...
    init: function () {
      for (var i in SUGAR.contextMenu.objects) {
        ...
        SUGAR.contextMenu.objectTypes[SUGAR.contextMenu.objects[i]["objectType"]]["objects"].push(
  ...
  return {
    ...
    addToFavorites: function (itemClicked, metaData) {
      ...
      var cObj = YAHOO.util.Connect.asyncRequest(
        "GET",
        "index.php?to_pdf=true&module=Home&action=AddToFavorites&target_id=" +
          metaData["id"] +
          "&target_module=" +
          metaData["module"],
...
function open_popup(
  ...
  if (initial_filter != "") {
    ...
    popupName = initial_filter.replace(/[^a-z_0-9]+/gi, "_");
  ...
  if (typeof name_to_value_array != "undefined" && name_to_value_array["account_id"]) {
    ...
    for (var the_key in name_to_value_array) {
      if (the_key == "toJSON") {
        ...
        if (
          ...
          document.getElementById(the_key + "_label") &&
          !the_key.match(/account/)
        ) {
          var data_label = document
            .getElementById(the_key + "_label")
            .innerHTML.replace(/\n/gi, "")
            .replace(/<\/?[^>]+(>|$)/g, "");
          label_str += data_label + " \n";
          label_data_str += data_label + " " + displayValue + "\n";
          if (window.document.forms[form_name].elements[the_key]) {
            current_label_data_str +=
              data_label + " " + window.document.forms[form_name].elements[the_key].value + "\n";
          }
        }
      }
    }
    if (label_data_str != label_str && current_label_data_str != label_str) {
      if (typeof popupConfirm != "undefined") {
        ...
        confirm(SUGAR.language.get("app_strings", "NTC_OVERWRITE_ADDRESS_PHONE_CONFIRM") + "\n\n" + label_data_str)
      ) {
        set_return_basic(popup_reply_data, /\S/);
      } else {
        set_return_basic(popup_reply_data, /account/);
...
SUGAR.append(SUGAR.util, {
  ...
  isLoginPage: function (content) {
    ...
    if (
      content.substr(0, loginPageStart.length) == loginPageStart &&
      content.indexOf("<html>") != -1 &&
      content.indexOf("login_module") != -1
    ) {
  ...
  innerText: function (el) {
    ...
    for (var i in el.childNodes) {
      ...
      if (typeof c.nodeName == "string" && c.nodeName == "#text") t += c.nodeValue;
      else t += SUGAR.util.innerText(c);
    }
    return t;
  },
  ...
  closeActivityPanel: {
    show: function (module, id, new_status, viewType, parentContainerId) {
      if (SUGAR.util.closeActivityPanel.panel) SUGAR.util.closeActivityPanel.panel.destroy();
        ...
        buttons: [
          {
            ...
            handler: function () {
              if (SUGAR.util.closeActivityPanel.panel) SUGAR.util.closeActivityPanel.panel.hide();
  ...
  setEmailPasswordDisplay: function (id, exists, formName) {
    ...
    if (exists) {
      ...
      link.style.display = "";
      if (typeof formName != "undefined") removeFromValidate(formName, id);
    } else {
      pwd.disabled = false;
      pwd.style.display = "";
  ...
  arrayIndexOf: function (arr, val, start) {
    if (typeof arr.indexOf == "function") return arr.indexOf(val, start);
...
function convertReportDateTimeToDB(dateValue, timeValue) {
  ...
  if (date_match != null && time_match != null) {
    ...
    return (
      date_match[date_reg_positions["Y"]] +
      "-" +
      date_match[date_reg_positions["m"]] +
      "-" +
      date_match[date_reg_positions["d"]] +
      " " +
      time_match[1] +
      ":" +
      time_match[2] +
