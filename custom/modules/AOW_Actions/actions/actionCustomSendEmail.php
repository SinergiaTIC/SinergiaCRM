...
class actionCustomSendEmail extends actionBase
{
    ...
    public function run_action(SugarBean $bean, $params = array(), $in_save = false)
    {
        ...
        if (isset($params['individual_email']) && $params['individual_email']) {
            foreach ($emails['to'] as $email_to) {
                ...
                // if (!$this->sendEmail($outputSmtp, array($email_to), $emailTemp->subject, $emailTemp->body_html, $emailTemp->body, $bean, $emails['cc'], $emails['bcc'], $attachments, $fromName, $replyto)) {
                // $templateData, $address, $name, $mailerName = 'system', $user = '')
                if (!$this->sendEmail($emailTemp, $email_to, 'personName', $outputSmtp, '')) {
                    $ret = false;
            ...
            // TODOEPS Tractar mails no individuals
            // if (!$this->sendEmail($outputSmtp, $emails['to'], $emailTemp->subject, $email_body_html, $emailTemp->body, $bean, $emails['cc'], $emails['bcc'], $attachments, $fromName, $replyto)) {
            if (!$this->sendEmail($emailTemp, $email_to, 'personName', $outputSmtp, '')) {
                $ret = false;
                $this->lastEmailsFailed++;
            } else {
    ...
    // public function sendEmail($mailerName, $emailTo, $emailSubject, $emailBody, $altemailBody, SugarBean $relatedBean = null, $emailCc = array(), $emailBcc = array(), $attachments = array(), $fromName, $replyto)
    public function sendEmail($templateData, $address, $name, $mailerName = 'system', $user = '')
    {
        if (empty($address)) {
            return false;
        }
        ...
        $outboundEmail = $outboundEmail->getMailerByName('', $mailerName);
        
        $mail->From = $outboundEmail->smtp_from_addr;
        $mail->FromName = $outboundEmail->smtp_from_name;
