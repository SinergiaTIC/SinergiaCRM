...
class CustomAOS_InvoicesController extends AOS_InvoicesController
{
    ...
    /**
     * Action to select payments and create an invoice from them.
     *
     * Shows a selector to choose payments, then creates an invoice with those payments.
     */
    public function action_selectPaymentsToInvoice()
    {
        global $mod_strings;

        if (!empty($_POST['confirm_create'])) {
            $this->processCreateFromPayments();
        } else {
            $this->view = 'selectpaymentstoinvoice';
        }
    }

    private function processCreateFromPayments()
    {
        global $mod_strings;

        $paymentIds = $_POST['payment_ids'] ?? array();
        if (empty($paymentIds)) {
            SugarApplication::appendErrorMessage(AOS_InvoicesUtils::getStyledErrorAlert($mod_strings['LBL_NO_PAYMENTS_SELECTED']));
            SugarApplication::redirect('index.php?module=AOS_Invoices&action=index');
            return;
        }
        ...
        if (empty($payments)) {
            SugarApplication::appendErrorMessage(AOS_InvoicesUtils::getStyledErrorAlert($mod_strings['LBL_NO_PAYMENTS_FOUND']));
            SugarApplication::redirect('index.php?module=AOS_Invoices&action=index');
            return;
        }

        if (!CreateInvoiceFromPayments::validateSamePayer($payments)) {
            SugarApplication::appendErrorMessage(AOS_InvoicesUtils::getStyledErrorAlert($mod_strings['LBL_PAYER_MISMATCH_ERROR']));
            SugarApplication::redirect('index.php?module=AOS_Invoices&action=selectPaymentsToInvoice');
            return;
        }

        $alreadyInvoiced = CreateInvoiceFromPayments::validatePaymentsNotInvoiced($payments);
        if (!empty($alreadyInvoiced)) {
            SugarApplication::appendErrorMessage(AOS_InvoicesUtils::getStyledErrorAlert($mod_strings['LBL_PAYMENT_ALREADY_INVOICED'] . ': ' . implode(', ', $alreadyInvoiced)));
            SugarApplication::redirect('index.php?module=AOS_Invoices&action=selectPaymentsToInvoice');
            return;
        }
        ...
        if ($result['success']) {
            SugarApplication::appendSuccessMessage(AOS_InvoicesUtils::getStyledSuccessAlert($mod_strings['LBL_INVOICE_CREATED_SUCCESS']));
