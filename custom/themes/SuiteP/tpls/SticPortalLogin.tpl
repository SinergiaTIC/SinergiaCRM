...
<body>
  <div class="card">
    <div class="logo">
      <img src="{/literal}{$LOGO_URL|escape}{literal}" alt="{/literal}{$TITLE|escape}{literal}">
    </div>
  ...
  {/literal}
    ...
    {if $MAGIC_ENABLED}
      <div class="tabs">
        <button id="tab-pw" class="{if $MODE neq 'magic_link'}active{/if}"
          onclick="switchTab('password')">Password</button>
        <button id="tab-ml" class="{if $MODE eq 'magic_link'}active{/if}" onclick="switchTab('magic_link')">Magic
          Link</button>
      </div>
    {/if}
    <form method="post">
      <div class="honeypot" aria-hidden="true"><input type="text" name="portal_hp" tabindex="-1" autocomplete="off">
      </div>
      ...
      {if $MAGIC_ENABLED}
        <div id="ct-magic" class="tab-content {if $MODE eq 'magic_link'}active{/if}">
          <p class="help">Enter your username and we'll send you a one-click login link.</p>
            <label>Username</label><input type="text" name="username_magic"
              onchange="document.querySelector('[name=username]').value=this.value">
