...
class SticPrivateAreaUtils
{
    ...
    public static function processBeforeSave($bean)
    {
        ...
        if (!self::isPortalEnabledNow($bean) && self::wasPortalEnabled($bean)) {
            ...
            $fetchedPassword = (string)($bean->fetched_row['stic_portal_password_c'] ?? '');
            $bean->_stic_plain_pa_password = '';
            if ($fetchedPassword !== '') {
                $bean->stic_portal_password_c = $fetchedPassword;
            } elseif ($storedPassword !== '') {
                $bean->stic_portal_password_c = $storedPassword;
            }
            return;
        }
        ...
        if ($isBeingEnabled && !$hasSubmittedPassword) {
            ...
            if ($fetchedPassword !== '') {
                ...
                $bean->stic_portal_password_c = $generatedPassword;
            }
            ...
            if ($fetchedPassword !== '' && $submittedPassword === $fetchedPassword && $storedPassword !== '') {
                ...
                $bean->stic_portal_password_c = $submittedPassword;
            }
        }
    ...
    {
        return (bool)($bean->fetched_row['stic_portal_enable_c'] ?? false);
    }
